#' Build pomp object for Model 1 - Sud Est
#'
#' Generate a class \sQuote{pomp} object for fitting to epidemic/endemic Haiti cholera data jointly.
#' This function is created to see if it can be used as an improvement to model 3
#' for Sud Est, which has trouble explaining the data from this department.
#'
...
#' @export
haiti1_SudEst <- function(vacscen = 'id0', breakpoint = 232, rho_flag = FALSE,
                         tau_flag = FALSE, sig_sq_flag = FALSE, beta_flag = FALSE,
                         nu_flag = FALSE) {
  ...
  if (depts > 1) {
    ...
    foi <- "double foi = pow(I + imp, nu) * mybeta / pop; \n "
  }
  ...
  param_trans <- pomp::parameter_trans(
    log = c(log_pars, "sigma", "gamma", "mu", "delta", "alpha", "imp"),
  ...
  param_names <- c(param_names, "betat", "imp")  # Adding trend parameter, should default to zero.
  pars <- c(pars, 'betat' = 0, 'imp' = 0)
